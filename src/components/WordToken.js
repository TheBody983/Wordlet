...
const WordToken = (props) => {
    const { user } = useAuth( )
    const { getTokenData, transferWordToken, buyWordtoken, listTokenForSale, removeTokenFromSale, getTokenPrice, updateTokenPrice } = useUser( )
    ...
    return (
        <div className="card" key={props.tokenId}>
            ...
            <div>
                <label>Prix : </label>
                <input type="text" id="price" placeholder="Entrez un prix"/>
                <button  onClick={() => updateTokenPrice(props.tokenId, document.getElementById("price").value)}>Changer le prix</button> 
            </div>
