...
$query = "
(select mot from `lexique_francais` where cgram regexp 'NOM' and freqlivres > 10 and mot not regexp ' ' order by rand() limit 6)
union
(select mot from `lexique_francais` where cgram regexp 'VER' and freqlivres > 10 and mot not regexp ' ' order by rand() limit 3)
union
(select mot from `lexique_francais` where cgram regexp 'ART|PRO' and freqlivres > 200 and mot not regexp ' ' order by rand() limit 2)
union
(select mot from `lexique_francais` where cgram regexp 'CON|PRE' and freqlivres > 10 and mot not regexp ' ' order by rand() limit 2)
union
(select mot from `lexique_francais` where cgram regexp 'ADV|ADJ' and freqlivres > 20 and mot not regexp ' ' order by rand() limit 2);
"
